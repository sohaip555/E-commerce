            ...
            <div class="flex flex-wrap mb-24 -mx-3">
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                            ...
                            @foreach($categories as $category)
                                <li class="mb-4" wire:key="{{$category->id}}">
                                    <label for="{{$category->slug}}" class="flex items-center dark:text-gray-400 ">
                                        <input type="checkbox" wire:model.live="categories_selected"
                                               value="{{$category->id}}" for="{{$category->slug}}"
                            ...
                            @foreach($brands as $brand)
                                <li class="mb-4" wire:key="{{$brand->id}}">
                                    <label for="{{$brand->slug}}" class="flex items-center dark:text-gray-300" >
                                        <input type="checkbox" wire:model="brands_selected"
                                               value="{{$brand->id}}" for="{{$brand->slug}}"
                                               id="{{$brand->id}}" class="w-4 h-4 mr-2">
                        ...
                        <ul>
                            <li class="mb-4">
                                <label for="" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" wire:model.live="featured_product" class="w-4 h-4 mr-2">
                                    <span class="text-lg dark:text-gray-400">Featured Product</span>
                                </label>
                            </li>
                            <li class="mb-4">
                                <label for="" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" wire:model.live="on_sale" class="w-4 h-4 mr-2">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6  dark:border-gray-400">
                            {{\Illuminate\Support\Number::currency($price_range)}}
                        </div>
                        <div>
                            ...
                            <div class="flex justify-between ">
                                <span class="inline-block text-lg font-bold text-blue-400 ">{{\Illuminate\Support\Number::currency(1000)}}</span>
                                <span class="inline-block text-lg font-bold text-blue-400 ">{{\Illuminate\Support\Number::currency(500000)}}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="px-3 mb-4">
                        <div class="items-center justify-between hidden px-3 py-2 bg-gray-100 md:flex dark:bg-gray-900 ">
                            <div class="flex items-center justify-between">
                                <select wire:model.live="sort_by" name="" id="" class="block w-40 text-base bg-gray-100 cursor-pointer dark:text-gray-400 dark:bg-gray-900">
